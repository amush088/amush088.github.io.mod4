...
function populateListProductChoices(slct1, slct2) {
    ...
    optionArray.sort((a, b) => (a[1] > b[1]) ? 1 : -1)
...
function displayProducts(optionArray, slct2) {
	...
	for (i = 0; i < optionArray.length; i++) {
		...
		productImage.src = optionArray[i][7];
...
function selectVegetable(cb, slct2 ) {
	if(cb.checked) {
		var optionArrayMod = [];
		//console.log(optionArray.length);
 ...
 function selectMeat(cb, slct2 ) {
  ...
  function selectBeverage(cb, slct2 ) {
	if(cb.checked) {
		var optionArrayMod = [];
		//console.log(optionArray.length);
